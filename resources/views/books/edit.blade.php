<x-app-layout>
    <div class="w-full ">
        <h1 class="text-primary font-bold text-4xl text-center">Edit Buku</h1>
        <div class="md:w-[60%] w-full mx-auto mt-10">
            <form action="/books/{{ $book->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="image w-full h-60 bg-primary rounded-lg mb-5"
                    style="background-image: url({{ asset($book->image) }}); background-size: cover; background-position: center;">
                    ...
                    <input type="file" name="image" id="image" class="hidden" accept="image/*"
                        value="{{ old('image', $book->image) }}" onchange="previewImage(event)">
                </div>
                ...
                <div class="md:flex md:mb-5">
                    ...
                    <input type="text" id="stock" name="stock" value="{{ old('stock', $book->stock) }}"
                        class="lg:w-1/3 w-full text-primary lg:text-xl text-lg  border border-primary  px-2 py-3 rounded-md focus:ring-1 focus:ring-primary focus:outline-none md:me-2 mb-5 md:mb-0"
                ...
                <textarea name="description" id="" rows="10"
                    ...
                    placeholder="Deskripsi Buku Ini...">{{ $book->description->description }}</textarea>
                <button class="px-4 w py-3 bg-primary text-accent font-bold rounded-lg">Edit Buku</button>
            </form>
        </div>
